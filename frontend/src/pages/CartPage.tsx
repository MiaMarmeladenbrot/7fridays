  ...
  return (
    <main className="mt-28 px-10 flex flex-col items-center">
      ...
      {data?.cart.items && data?.cart?.items?.length < 1 ? (
        <p className="font-poppins-reg mb-5">
          Nothing found yet?{" "}
          <Link className="text-[#4852af]" to="/">
            {" "}
            Continue shopping!
          </Link>{" "}
          😊
        ...
        data?.cart.items.map((singleItem: CartItem) => (
          <div
            key={singleItem.product.id}
            className="flex items-center gap-5 justify-between min-w-60 mb-5"
          >
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="font-poppins-reg">
                  {singleItem.quantity}x {singleItem.product.name}
                </p>
                <p className="font-poppins-reg">
                  {singleItem.product.price * singleItem.quantity} €
                </p>
              </div>
            </div>
      ...
      <div>
        <button className="font-poppins-reg bg-green-600 py-1 px-2 rounded-lg mb-3 text-white mr-10">
          Buy now
        </button>
        <ClearCart />
      </div>
